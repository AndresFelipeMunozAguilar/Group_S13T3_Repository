...
- Como lo implementé: La siguiente compuerta la implementé usando los siguientes chips: Mux16, Register y Inc16. El Mux16 para tomar el lugar de la funcionalidad de los condicionales (“If”), el chip register para almacenar la palabra de 16 bits cundo le fuera dicho y, finalmente, el Inc16 para incrementar en 1 unidad la palabra de 15 bits cuando la variable “Inc” lo indicara. Primero, hay un Mux16 que hace las funciones de incrementar el valor de la salida por 1 unidad, que, en caso de que el incremento sea 1, “pasa” el valor que viene del pin a, es decir, el pin que tiene la salida del “inc16” que, anteriormente, estaba conectado a la salida del register; así, cualquier salida que tenga el register, el Inc16 lo incrementará 1 unida; en cambio, si el incremento es 0, entonces, querrá decir que todas las demás variables (Load, inc, reset) son 0 y, por tanto, sólo pasará el estado anterior de la salida del registro, cumpliendo así que: out[t+1] = out[t]. Luego, hay un Mux16 que sirve para guardar la entrada que haya; para esto, tiene un selector que depende de la variable “load” que, en caso de ser 1, pasa el input del pin b; es decir, la input del usuario; esto, lo pasa al register y, como la variable load del register es siempre 1, eso quiere decir que guardará el valor del input y lo “sacará” como la salida del circuito entero; en caso contrario, tomará la entrada del pin a; es decir, la salida del Mux16 que evalúa la función de carga que, en otras palabras, significa que, cuando “load” es 0, se pasa a evaluar la condición de “inc”. Luego, hay un Mux16 que haces las funciones de “reset”; si “reset” es igual a 1, entonces, el Mux16 toma la entrada del pin b, es decir, toma el valor de false y lo pasa al register para que lo guarde indefinidamente, hasta que cambie el valor de reset; de otra forma, si “reset” es igual a 0, toma la entrada del pin a; es decir, la salida del Mux16 que hace las veces de “load”, lo que, en otras palabras, significa que si “reset” es igual a 0, el circuito pasa a revisar si se cumple la condición de que “load” sea igual a 1. Finalmente, la salida del register es la salida de todo el circuito; pero, la entrada del Inc16 es la salida del register; así, cada vez que el register almacene un valor, el Inc16 lo incrementará en 1 unidad; preparado para cuando la variable “inc” lo llame para entregar el valor de la salida anterior incrementada en 1 unidad. Esto se entiende mejor con la siguiente imagen:   
<image src="dibujo_arquitectura.jpg" alt="Dibujo de las compuertas">   